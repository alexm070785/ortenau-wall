...
// User aus Context ziehen (kompatibel zu verschiedenen Netlify-Runtimes)
function getUserFromContext(ctx) {
  return ctx?.user || ctx?.clientContext?.user || null;
}

// Normalisierung alter Bildpfade
function norm(u) {
  if (!u) return u;
